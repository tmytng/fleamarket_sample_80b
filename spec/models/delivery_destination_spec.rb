...
describe DeliveryDestination do
    ...
    it "苗字が空の場合登録ができない" do
        ...
        expect(delivery_destination.errors[:family_name]).to include("can't be blank")
    end

    it "名前が空の場合登録ができない" do
        delivery_destination = build(:delivery_destination, first_name: nil)
        delivery_destination.valid?
        expect(delivery_destination.errors[:first_name]).to include("can't be blank")
    end

    it "苗字（カナ）が空の場合登録ができない" do
        delivery_destination = build(:delivery_destination, family_name_kana: nil)
        delivery_destination.valid?
        expect(delivery_destination.errors[:family_name_kana]).to include("can't be blank")
    end

    it "名前（カナ）が空の場合登録ができない" do
        delivery_destination = build(:delivery_destination, first_name_kana: nil)
        delivery_destination.valid?
        expect(delivery_destination.errors[:first_name_kana]).to include("can't be blank")
    end
    ...
    it "住所（address）が空の場合登録ができない" do
        ...
        delivery_destination.valid?
        expect(delivery_destination.errors[:address]).to include("can't be blank")
    end
